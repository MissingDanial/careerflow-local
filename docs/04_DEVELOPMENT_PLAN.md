...
截至 2026-07-13，M13“质量基线与可回放工作流”已完成，M14 已完成单岗位真实打招呼授权协议和 options 工作台收敛，M15 已完成 ProfileAgent 持久化对话基础，M16 已完成真实模型 Agent 质量闭环并通过 9 岗位 x 3 次正式评测，M16.1 已完成真实岗位 Shadow 评审基础。首批用户 Shadow run、真实 BOSS 页面 canary、上传简历和投递仍保持关闭。
...
1. 完成 M16.1，并用本地真实 JD 做 Shadow 评审；持续把匿名化误筛、漏筛、Schema 和 Audit 失败加入评测集。
2. 配置供应商实际 input/output 单价后再启用费用门禁。
3. 在用户对具体岗位再次明确授权时，单独执行 M14.1 真实打招呼 canary；Agent 或 Shadow 评测通过不构成 BOSS 动作授权。
4. 两项门禁都通过后，再进入 M14.3 简历选择/上传 POC；真实投递仍最后进行。
