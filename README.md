...
- M13.2 replaces the centralized SQLite schema block with ordered SQL migrations, checksum history, pre-migration `VACUUM INTO` backups, and automatic restoration after migration failure; the current schema sequence is `001-012`.
...
M1-M12 的主流程已经形成，M13.1-M13.5 已完成仓库质量基线、SQLite 有序迁移、不可变工作流输入、application 状态迁移收敛和 Agent 固定评测。下一阶段顺序是：

```text
扩充真实匿名失败样本并保持 Agent 评测基线
-> 基于证据与质量结果重新评估真实 BOSS 发送/上传/投递动作
```
