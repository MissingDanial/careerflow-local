...
- M13.2 replaces the centralized SQLite schema block with ordered SQL migrations, checksum history, pre-migration `VACUUM INTO` backups, and automatic restoration after migration failure; the current schema sequence is `001-014`.
...
运行后会在 `server/data/agent-evaluation/` 生成完整 JSON/Markdown 报告。该目录默认不提交，公开仓库保留匿名固定集、评测代码和上表基线结果。固定集当前使用确定性 rules mode，主要用于发现代码、规则和 Agent 契约回归；它不等同于真实 LLM 多次采样质量、真实用户投递成功率或 BOSS 自动化成功率。
...
M1-M12 的主流程已经形成，M13.1-M13.5 已完成质量基线，M14.1 已完成单岗位真实打招呼授权协议与本地 fixture 验证。下一阶段顺序是：

```text
在用户明确确认下运行 1 个真实 BOSS 打招呼 canary
-> 运行真实模型 Agent 评测并记录成本、延迟与方差
-> 通过两项门禁后再做简历选择/上传 POC
-> 真实投递 canary 最后进行
```
