...
- M13.2 replaces the centralized SQLite schema block with ordered SQL migrations, checksum history, pre-migration `VACUUM INTO` backups, and automatic restoration after migration failure; the current schema sequence is `001-015`.
...
扩展 popup 只保留主操作：开始岗位信息采集、暂停、重试。options 页默认是 application 岗位队列和唯一上下文操作；ProfileAgent 画像维护位于“个人经历”，后端配置、岗位偏好和模板位于“设置”，采集质量、任务诊断、最近异常、待补 JD、Agent 节点和旧手动工具统一折叠到“高级诊断”。真实打招呼从工作台进入两步确认弹窗，后端短时策略、一次性授权、每日额度和不自动重试规则保持不变。
...
M1-M12 的主流程已经形成，M13.1-M13.5 已完成确定性质量基线，M14.1 已完成单岗位真实打招呼授权协议与本地 fixture 验证，M15.1 已完成 ProfileAgent 持久化对话基础，M16 已通过真实模型 9 岗位 x 3 次质量评测。下一阶段顺序是：

```text
继续扩充匿名真实失败样本并监控 M16 质量门禁
