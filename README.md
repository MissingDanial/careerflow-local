...
- M13.2 replaces the centralized SQLite schema block with ordered SQL migrations, checksum history, pre-migration `VACUUM INTO` backups, and automatic restoration after migration failure; the current schema sequence is `001-016`.
...
- 工作台可按岗位状态执行“评估岗位、生成定制简历、查看并审批、准备打招呼、发送一次”等上下文操作；简历详情与审批按需在弹窗中打开，不再平铺所有 Agent 节点按钮。
...
当前实现仍不是完整自动投递系统。真实打招呼只开放了默认关闭的单岗位金丝雀协议，尚未在 README 中宣称通过真实 BOSS 页面验证；真实简历上传、批量打招呼、后台自动发送和真实投递仍未开放。
...
真实模型配置分两层：环境变量或本机 `gpt5.5.txt` 提供凭据和 base URL；`boss-model.local.json` 只覆盖项目实际使用的 model、wire API、超时和重试。后者被 Git 忽略，可参考 `boss-model.example.json`。浏览器扩展不读取或保存 API Key。
...
6. 在岗位列表页点击扩展图标，使用“开始岗位信息采集 / 暂停 / 重试”。
7. 打开扩展详情页后，默认进入“Boss Find 工作台”；在“个人经历”中可与 ProfileAgent 多轮对话、确认画像修改并生成版本化职业上下文，后端配置和高级诊断在“设置”。
...
扩展 popup 只保留主操作：开始岗位信息采集、暂停、重试。options 页默认是 application 岗位队列和唯一上下文操作；ProfileAgent 画像维护位于“个人经历”，后端配置、岗位偏好和模板位于“设置”，采集质量、任务诊断、最近异常、待补 JD、Agent 节点、Shadow 评审和旧手动工具统一折叠到“高级诊断”。真实打招呼从工作台进入两步确认弹窗，后端短时策略、一次性授权、每日额度和不自动重试规则保持不变。
...
M1-M12 的主流程已经形成，M13.1-M13.5 已完成确定性质量基线，M14.1 已完成单岗位真实打招呼授权协议与本地 fixture 验证，M15.1 已完成 ProfileAgent 持久化对话基础，M16 已通过真实模型 9 岗位 x 3 次质量评测，M16.1 已实现真实岗位 Shadow 评审与人工纠正基础。下一阶段顺序是：

```text
在确认画像后运行首批本地真实 JD Shadow 评审并沉淀失败样本
-> 在用户明确确认下运行 1 个真实 BOSS 打招呼 canary
-> 通过 Agent 与真实打招呼两项门禁后再做简历选择/上传 POC
-> 真实投递 canary 最后进行
```
