...
环境要求：Node.js 24+、Chrome 或 Edge，以及用户自行登录的 BOSS 直聘账号。岗位采集和本地规则流程不强制配置模型；ProfileAgent 对话和默认的模型简历生成流程需要 OpenAI 兼容接口。
...
2. 打开 `chrome://extensions/` 或 `edge://extensions/`，开启开发者模式，选择“加载已解压的扩展程序”，加载仓库内的 `extension` 目录。

3. Windows 用户在扩展加载完成后安装一键启动器，并重新加载扩展：

   ```powershell
   npm run native:install
   ```

   如果安装器未自动识别扩展 ID，可从扩展管理页复制 32 位 ID 后执行：

   ```powershell
   powershell -NoProfile -ExecutionPolicy Bypass -File scripts/install-native-host.ps1 -ExtensionId <extension-id> -Browser Chrome
   ```

   macOS/Linux 或不使用 Native Host 时，直接运行 `npm run server` 并保持终端开启。健康检查地址为 `http://127.0.0.1:8787/health`。

4. 点击扩展齿轮进入“设置 -> 基础模型服务”，填写 Base URL、模型、协议和 API Key，保存后点击“测试连接”。凭据只写入被 Git 忽略的 `server/data/model-provider.local.json`。若要使用 M18 推荐的节点路由，先执行：

   ```powershell
   Copy-Item boss-model.example.json boss-model.local.json
   ```

5. 在“个人经历”上传简历并确认画像事实；打开并登录 BOSS 岗位列表，在 popup 选择目标队列后点击“开始岗位信息采集”；最后进入工作台依次处理“完整 JD / 岗位筛选 / 定制简历 / 人工联系与投递”。

遇到登录失效、验证码或安全验证时，插件会暂停。请用户在页面中手动完成验证后点击“重试”，项目不会绕过平台安全机制。
...
- M18 完成 Agent 延迟与模型路由优化：相同输入可复用完整工作流，强制重跑仍复用 Screening；默认速度路由只让 Resume/Revision 使用真实模型，Screening/Fit/Claim/Audit 使用确定性门禁；提示载荷去重、隐藏模板字段不进入质量门禁、无证据缺口不触发修订，DOCX 只渲染最终版本。默认模板隐藏独立技能区时，只有同时具备画像技能和对应项目事实的 JD 必要项才会写入可见项目技能行并保留双重来源映射。
...
真实模型凭据仍从环境变量或被忽略的 `gpt5.5.txt` 读取。项目级非敏感覆盖写在被忽略的 `boss-model.local.json`，仓库提供 `boss-model.example.json`。当前供应商已实测支持 `gpt-5.4-mini + chat` 和 `gpt-5.5 + responses`；默认速度路由使用后者生成/修订简历，其余质量门禁保持确定性。兼容层返回数字字符串 ID 时只对纯十进制正整数做受控归一化，之后仍执行画像 ID、sourceFact 和 Claim 校验。Chat 双层 JSON 会受控解包，429/5xx 使用指数退避并记录每次 attempt。

### M18 Agent 延迟与模型路由

同一份盛大 AI 产品实习 JD 与同一份已确认画像的本地隔离验证结果：

| 路由 | 首次闭环 | 模型调用 | Revision | Fit | Claim | Audit |
| --- | ---: | ---: | ---: | ---: | ---: | --- |
| `gpt-5.5 responses` 仅 Resume，其他节点规则化 | 44.3 秒 | 1 | 0 | 67，无 blocker | 54/54 支持 | approve |
| `gpt-5.4-mini chat` 仅 Resume/Revision，其他节点规则化 | 48.7 秒 | 2 | 1 | 62，无 blocker | 54/54 支持 | approve |

GPT-5.5 首稿输入由 8,157 token 压缩到 4,561 token，单次 Resume 从 76.5 秒降到 43.8 秒；重复运行的规则 smoke 从 184-258ms 降到约 13ms，缓存命中时模型调用为 0。以上真实模型数据只是一组同 JD 工程对照，用来决定本机默认路由，不替代 M16 的 27 样本质量评测，也不代表真实投递成功率。

最终仓库回归覆盖 131 个 JavaScript 文件语法检查、73/73 个 CI smoke 和 32/32 个 workflow smoke。M10 LangGraph 样例在默认隐藏技能区模板下完成 1 次证据约束修订，将已确认的 `PRD` 写入可见项目技能并保留技能/项目事实映射；最终 Fit blocker 为 0，Claim、Audit、Render QA 和 DOCX 输出全部通过。无法关联到具体项目事实的技能不会为了提高分数被塞入项目经历。
